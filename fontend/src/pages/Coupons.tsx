            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {coupons.map(coupon => (
                    <div key={coupon.id} className="border-l-4 border-primary bg-white shadow-sm p-4 rounded-r-xl flex items-center justify-between">
                      ...
                      {soldOut && <p className="text-xs text-red-500 font-semibold mt-1">Mã đã hết lượt sử dụng</p>}
                      {!soldOut && expired && <p className="text-xs text-orange-500 font-semibold mt-1">Mã đã hết hạn</p>}
                        </div>
                        <button
