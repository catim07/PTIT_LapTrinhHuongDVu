...
const Cart: React.FC = () => {
  ...
  const handleUpdateQuantity = (branchProductId: string, newQuantity: number) => {
    const safeQuantity = toSafeQuantity(newQuantity);
  ...
  if (!isAuthenticated) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center bg-background-light dark:bg-background-dark font-display">
        ...
        <p className="text-slate-500 mb-8">Đăng nhập để xem giỏ hàng của bạn</p>
  ...
  if (status === 'loading') {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center bg-background-light dark:bg-background-dark font-display">
        ...
        <h2 className="text-2xl font-bold mb-4">Đang tải giỏ hàng</h2>
  ...
  if (!currentBranchId) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center bg-background-light dark:bg-background-dark font-display">
        ...
        <p className="text-slate-500 mb-8">Vui lòng chọn chi nhánh để xem giỏ hàng</p>
      </main>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center bg-background-light dark:bg-background-dark font-display">
        ...
        <p className="text-slate-500 mb-8">Hãy thêm sản phẩm vào giỏ hàng của bạn nhé!</p>
        ...
        <div className="flex-1 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-3xl font-extrabold tracking-tight">
              Giỏ hàng của tôi <span className="text-lg font-normal text-slate-500">({items.length} sản phẩm)</span>
            </h2>
            {currentBranch && (
                ...
                <div className="w-32 h-32 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden shrink-0 relative">
                  ...
                  {(bp?.is_expired || bp?.is_expiring_soon) && (
                    <div className={`absolute top-0 right-0 left-0 text-center text-[9px] font-bold text-white py-0.5 ${bp.is_expired ? 'bg-red-600' : 'bg-orange-500'}`}>
                      {bp.is_expired ? 'HẾT HẠN' : 'CẬN DATE'}
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0 w-full">
                  ...
                  <div className="flex flex-col gap-0.5 mb-3">
                    <p className="text-[11px] font-mono text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded w-max">
                      ...
                      <span className="font-bold">Danh mục:</span> {bp?.category_name || productCore?.category_name || (item as any).category_name || 'N/A'}
                      <span className="mx-2 text-slate-300">|</span> 
                      <span className="font-bold">NCC:</span> {bp?.supplier_name || productCore?.supplier_name || (item as any).supplier_name || 'N/A'}
                    </p>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center bg-primary/5 rounded-lg p-1">
                      <button
                        onClick={() => safeQuantity > 1 && handleUpdateQuantity(item.branch_product_id, safeQuantity - 1)}
                      ...
                      <button
                        onClick={() => safeQuantity < maxLimit && handleUpdateQuantity(item.branch_product_id, safeQuantity + 1)}
                        className="w-8 h-8 flex items-center justify-center hover:bg-primary/10 rounded-md transition-colors opacity-100"
                        title={safeQuantity >= maxLimit ? `Tối đa ${maxLimit} sản phẩm` : ""}
                      >
                ...
                <div key={idx} className="bg-green-50 dark:bg-green-900/10 p-3 rounded-lg border border-green-100 flex justify-between items-center mb-2">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-600 dark:text-green-400">Số lượng: {gift.quantity}</p>
                      </div>
                   </div>
                   <div className="text-green-700 font-bold">Miễn phí</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-lg border border-primary/10 sticky top-28">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">{shippingFee === 0 ? 'Miễn phí' : `${shippingFee.toLocaleString('vi-VN')}đ`}</span>
              </div>
            ...
            <div className="space-y-3">
              ...
              {appliedCoupon && !couponError && (
                 <p className="text-xs text-green-600 font-bold">Đã áp dụng mã: {appliedCoupon.code}</p>
              )}
            ...
            <div className="mt-6 flex items-center gap-3 p-4 bg-primary/5 rounded-xl border border-primary/10">
              ...
              <p className="text-xs font-medium text-slate-600 dark:text-slate-300">
                Miễn phí vận chuyển cho đơn hàng từ <span className="text-primary font-bold">300.000đ</span>.
              </p>
            </div>
          </div>
        </div>
