      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-10 px-4 lg:px-40">
        <div className="max-w-[480px] w-full flex flex-col items-center text-center gap-8">
          ...
          <div className="w-full flex flex-col gap-3">
            <button onClick={() => navigate('/payment')} className="flex h-14 w-full items-center justify-center rounded-xl bg-primary text-white text-base font-bold tracking-wide hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">{t('paymentFailed.retry')}</button>
            <button onClick={() => navigate('/cart')} className="flex h-14 w-full items-center justify-center rounded-xl bg-primary/10 text-primary text-base font-bold tracking-wide hover:bg-primary/20 transition-colors">{t('paymentFailed.backToCart')}</button>
          </div>
          ...
          <div className="w-full mt-4">
            <div className="flex flex-col items-start justify-between gap-4 rounded-xl border border-primary/20 bg-white dark:bg-background-dark/50 p-6 sm:flex-row sm:items-center">
              <div className="flex flex-col gap-1 text-left">
                <p className="text-slate-900 dark:text-slate-100 text-base font-bold">{t('paymentFailed.needHelp')}</p>
                <p className="text-slate-500 dark:text-slate-400 text-sm">{t('paymentFailed.contactCs')}</p>
              </div>
          ...
          {state.orderId && (
            <div className="flex flex-col gap-6 w-full pt-8 border-t border-primary/10">
              <p className="text-slate-400 text-xs font-medium uppercase tracking-widest">{t('paymentFailed.errorDetails')}</p>
              <div className="grid grid-cols-1 gap-4 text-left">
                <div className="flex justify-between items-center py-2 border-b border-primary/5">
                  <span className="text-slate-500 text-sm">{t('paymentFailed.orderId')}</span>
                  <span className="text-slate-900 dark:text-slate-100 text-sm font-semibold">#{state.orderId}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-primary/5">
                  <span className="text-slate-500 text-sm">{t('paymentFailed.time')}</span>
